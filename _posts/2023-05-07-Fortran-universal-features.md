...
* Fortranには部分配列という機能があり、C/C++にはないので(Pythonにはある)説明していないが、
...
| 機能 | C/C++ | F77 | F90/95 | F03 | F08 | F18 |
| --- | --- | --- | --- | --- | --- | --- |
| 文字要素 | `x[i][j]` | `x(i)(j:j)` | (同左) | (同左) | (同左) | (同左) |
...
| 機能 | C/C++ | F77 | F90/95 | F03 | F08 | F18 |
| --- | --- | --- | --- | --- | --- | --- |
| 割当て | `x = malloc(sizeof(int)*N)` | `ALLOCATE(x(N))` | (同左) | (同左)<br>または<br>`ALLOCATE()` | (同左) | (同左) |

# ポインタ

* C/C++のポインタはアドレスを表現するものだが、Fortranでは単なる別名(エイリアス)を与えるだけ
...
* Fortranの多相性の例に書いた実装を実現するには、`output`関数が引数を`CLASS(point)`として受け取りSELECT TYPE構文で型ごとに処理を分ける必要がある
...
* 余りはなぜか実数型に対しても使える
* Fortran 90以降では`OPERATOR`文を使って演算子を再定義したり新しく定義することが可能
  * C++では再定義だけのはず
...
10 CONTINUE
        ...
        <td>(同左)<br>または<br><pre>DO i = 1, 10, 1
  a(i) = i
END DO</pre>
        </td>
        <td>(同左)</td>
...
* 外部関数を使うには`INTERNAL`文または`EXTERNAL`文が必要
